# coded by Muhamad Yunus
# RISKY BOT DAN IRAWAN NOB:v
# fb me : Yunus X Ganz
...
class about:
	...
	def tentang_sc(self):
		print("""
[*] Author : Muhamad Yunus
[*] Github : https://www.Bacot.com
[*] Facebook : Yunus X Ganz
[*] WhatsApp : +6289518044468
[*] Script Name : mbf-fb
[*] Version : 1.1
...
class Risky Bot:
	...
	def folower(self,hencet):
		try:
			...
			Irawan Nob=re.findall('" \/>\<div\ class\=\"..\"\>\<a\ href\=\"\/(.*?)\"\><span\>(.*?)\<\/span\>',kontol) 
			for softek in memek:
				if "&amp;refid=" in softek[0]:
					self.id.append(re.findall("id=(.*?)&",softek[0])[0]+"[YunusGanteng]"+softek[1])
				elif "profile.php?" in softek[0]:
					self.id.append(re.findall("id=(.*)",softek[0])[0]+"[YunusGanteng]"+softek[1])
				elif "?refid=" in softek[0]:
					self.id.append(re.findall("(.*?)\?refid=",softek[0])[0]+"[YunusGanteng]"+softek[1])
				else:
					self.id.append(softek[0]+"[YunusGanteng]"+softek[1])
						...
						for c in b:
							if "profile.php" in c[0]:
								...
								if d in self.id:
									...
									self.id.append(d+"[YunusGanteng]"+c[1])
							else:
		...
		try:
			...
			for softek in memek:
				if "profile.php?" in softek[1]:
					self.id.append(re.findall("id=(.*?)&amp;refid",softek[1])[0]+"[YunusGanteng]"+softek[2])
				else:
					self.id.append(re.findall("(.*?)\?refid=",softek[1])[0]+"[YunusGanteng]"+softek[2])
				print(f"\r[*] Mengumpulkan {len(self.id)} ID",end="")
				if len(self.id)==jum:
					true=True
					break
		...
		try:
			...
			for softek in memek:
				if "profile.php?" in softek[0]:
					...
					self.id.append(re.findall("\/(.*?)\?fref",softek[0])[0]+"[YunusGanteng]"+softek[1])
				print(f"\r[*] Mengumpulkan {len(self.id)} ID",end="")
	...
	def menta(self,hencet):
		try:
			...
			for softek in memek:
				if "?uid" in softek[0]:
					self.id.append(re.findall("uid\=(.*?)\&",softek[0])[0]+"[YunusGanteng]"+softek[1])
				else:
					self.id.append(re.findall("\/(.*?)\?fref",softek[0])[0]+"[YunusGanteng]"+softek[1])
				print(f"\r[*] Mengumpulkan {len(self.id)} ID",end="")
			if "Lihat selengkapnya" in kontol:
	...
	def reactpost(self,hencet):
		try:
			...
			if "Semua 0" in kontol:
				...
				for softek in memek:
					if "profile.php?" in softek[0]:
						self.id.append(re.findall("id=(.*)",softek[0])[0]+"[YunudGanteng]"+softek[1])
					else:
						self.id.append(re.findall("/(.*)",softek[0])[0]+"[YunusGanteng]"+softek[1])
					print(f"\r[*] Mengumpulkan {len(self.id)} ID",end="")
				if "Lihat Selengkapnya" in kontol:
				...
				if "profile.php?" in softek[0]:
					...
					if "/" in tol[-1:]:
						...
						if tol in self.id:
							...
							self.id.append(tol+"[YunusGanteng]"+softek[1])
				else:
					...
					if "/" in tol[-1:]:
						...
						if tol in self.id:
							...
							self.id.append(tol+"[YunussGanteng]"+softek[1])
				print(f"\r[*] Mengumpulkan {len(self.id)} ID",end="")
				if len(self.id)==jum:
					true=True
					break
	...
	def fuck(self):
		...
		if pilih in["l","L"]:
			...
			print("[*] Silahkan Hubungi WhatsApp Saya 089518044468");os.system("xdg-open http://wa.me/+6289518044468?text=assalamualaikum");input("[*] Enter Untuk Kembali Ke Menu > ");waktu(2);self.menu()
		elif pilih in["u","U"]:
			os.system("git pull")
			exit()
		elif pilih in["h","H"]:
	...
	def askk(self):
		...
		if njir in(""," "):
			...
			print("[*] Contoh : RiskyBot123,IrawanNob12345")
			while True:
				pwek=input("[?] Password : ")
				if pwek in(""," "):
					print("[!] Jangan Kosong Bro")
		...
		try:
			...
			if "mbasic_logout_button" in cek:
				...
				if "Laporkan Masalah" in cek:
					...
					mengontol=Yunus.ganteng(zxss(self.cok),self.url)
...
"""
Create By Muhamad yunus
GITHUB : https://github.com/MuhamadYunus
"""